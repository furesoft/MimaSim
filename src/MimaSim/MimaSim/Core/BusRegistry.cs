...
public static class BusRegistry
{
    ...
    public static void Activate(string id)
    {
        if (_activationDefinitions.ContainsKey(id))
        {
            ...
            foreach (var b in def.BusIDs) ActivateBus(b);
    ...
    public static void DeactivateAll()
    {
        foreach (var id in _ids)
        {
            ...
            Dispatcher.UIThread.InvokeAsync(() => { MainBus.Foreground = Brushes.Gray; });
        }
    }
    ...
    public static void LoadMapsFromConfig()
    {
        ...
        foreach (XmlNode item in root.ChildNodes)
        {
            ...
            foreach (XmlNode setter in item.ChildNodes)
                ...
                else if (setter.Name == "mainbus") activatorEntry.MainBus = int.Parse(setter.Attributes["value"].Value);
    ...
    {
        if (!_ids.ContainsKey(id)) _ids.Add(id, control);
    }

    public static void SetData(string id, object value)
    {
        if (_ids.ContainsKey(id))
            Dispatcher.UIThread.InvokeAsync(() => { _ids[id].Tag = value; });
    }
    ...
    public static void SetId(object target, string id)
    {
        if (target is BusControl bc)
            RegisterBus(id, bc);
        else if (target is ProgressBar mb) MainBus = mb;
    }

    private static void ChangeBusState(string id, BusState state)
    {
        if (_ids.ContainsKey(id))
            Dispatcher.UIThread.InvokeAsync(() => { _ids[id].State = state; });
    }
}